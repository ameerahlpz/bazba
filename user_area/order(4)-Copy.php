...
$status='pending';
			...
			$cart_query_price="SELECT cart_details.product_id AS product_ID, cart_details.invoice_num AS invoice_num, cart_details.quantity AS quantity_products, cart_details.price AS price, cart_details.total AS total  
					FROM cart_details 
					INNER JOIN products ON
					cart_details.product_id = products.product_ID";
					
					
			// $cart_query_price="SELECT cart_details.product_id, cart_details.invoice_num,cart_details.quantity, cart_details.price, cart_details.total, products.product_ID 		
			// from cart_details JOIN products ON cart_details.product_id=products.product_ID";

			//line 40 products.product_ID
			//$cart_query_price="Select * from cart_details JOIN products ON cart_details.product_id=products.product_ID"; 	// AND orders_pending.order_id=user_orders.order_id";	// where ip_address ='$get_ip_address'";
			$conn = mysqli_connect('localhost','root','','bazbastore');
...
$insert_complete_orders_products="Select products.product_ID, products.title FROM products INNER JOIN cart_details ON products.product_ID=cart_details.product_id";		
$conn=mysqli_connect("localhost", "root", "", "bazbastore");
